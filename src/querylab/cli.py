...
from querylab.engines.base import QueryResult, SQLExecutionError
...
from querylab.generation import ExerciseGenerationError
from querylab.grading.grader import (
    GradeResult,
    Grader,
    ReferenceSolutionError,
)
from querylab.llm import LLMProviderError
from querylab.models import Dialect, Difficulty, Exercise, ExerciseRequest
from querylab.services import generate_exercise
...
def _querylab_is_running(url: str) -> bool:
    try:
...
def _launch_web(host: str, port: int, *, open_browser: bool) -> None:
    ...
    if _querylab_is_running(url):
        console.print(f"[bold green]QueryLab[/bold green] is already running at {url}")
        if open_browser:
    ...
    if _port_is_in_use(host, port):
        raise ValueError(
            ...
            f"Try `querylab --web --port {port + 1}`."
        )

    console.print(f"[bold green]QueryLab[/bold green] is running at {url}")
    console.print("[dim]Press Ctrl+C to stop the local server.[/dim]")
    if open_browser:
        timer = threading.Timer(0.8, _open_web_browser, args=(url,))
        timer.daemon = True
        timer.start()
    uvicorn.run("querylab.web.app:app", host=host, port=port, log_level="info")
...
def main(
    ...
    try:
        ...
        console.print(
            f"[bold red]Could not start QueryLab:[/bold red] {escape(str(exc))}"
        )
        if not static:
            console.print(
                "[dim]Try `querylab --static` for the offline exercise.[/dim]"
            )
